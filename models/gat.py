...
class GAT(nn.Module):
    ...
    def forward(self, seq_h, adj):
        ...
        for i, gnn_layer in enumerate(self.gnn_layer_stack):
            seq_h = gnn_layer(seq_h, adj)

        return seq_h

if __name__ == "__main__":
    input_vertex = torch.randn(8, 12, 120)
    adj = torch.randn(8, 12, 12)
    model = GAT(input_dim=120, gnn_dim=8, num_layers=2, num_heads=15, dropout=0.1, leaky_alpha=0.2)
    seq_h = model(input_vertex, adj)
    print(seq_h.size())
